%PlotMotionBIDS_fmriprep(folder_BIDS)
% Creates summary figures of motion for each subject/task/session.
% This modified version reads motion from *_desc-confounds_timeseries.tsv generated by fmriprep
...
function PlotMotionBIDS_fmriprep(folder_BIDS)
    ...
    for t = 1:length(task_IDs)
        ...
        for s = 1:max_ses
            ...
            for f = 1:number_files
                %read
                tsv = readtable(files(f).filepath, 'FileType', 'text');
                
                %number of volumes
                files(f).volumes = height(tsv);
                
                %get measures
                files(f).trans_x_mm = tsv.trans_x;
                files(f).trans_y_mm = tsv.trans_y;
                files(f).trans_z_mm = tsv.trans_z;
                files(f).rot_x_deg = tsv.rot_x;
                files(f).rot_y_deg = tsv.rot_y;
                files(f).rot_z_deg = tsv.rot_z;
