...
%% same thing but with all pixels
FILENAME_SAVE = [FILENAME_SAVE '_all'];

%% Mean Colours
fprintf('\nCalculating average colours...\n')
colour_average = (cell2mat(cellfun(@(x) squeeze(mean(mean(x,1),2)), images, 'UniformOutput', false))')/255;

%% Plot Mean Colours
fprintf('\nCreating image of average colours (all)...\n')
fig = figure('Position', [1 1 1200 1000]);
num_col = ceil(sqrt(num_image));
if (num_col * (num_col-1)) >= num_image
    num_row = num_col - 1;
...
fp = [DIR_SAVE 'colour_averages_all' FILETYPE_IMAGE];
imwrite(frame2im(getframe(fig)), fp);

%% Difference Match
colour_average_all_diff_matrix = squareform(pdist(colour_average));

%% Simmilarity Model
colour_average_all_model = (colour_average_all_diff_matrix / -max(colour_average_all_diff_matrix(:))) + 1;

%% Model Figure
fprintf('\nCreating colour average (all) model figure...\n')
    clf
    imagesc(colour_average_all_model)
    axis image
    colormap jet
    colorbar
    caxis([0 1])
...
%% Save Model
fprintf('\nSaving colour average (all) model...\n')
fp = [DIR_SAVE FILENAME_SAVE '.mat'];
fprintf('Filepath: %s\n', fp);
save(fp, 'colour_average_all_model');
...
if any(image_is_collapse)
    ...
    %zero the diag
    colour_average_all_model_nodiag = colour_average_all_model;
    colour_average_all_model_nodiag([1:num_image] + [0:num_image:(num_image * (num_image-1))]) = nan;
    
    %init
    colour_average_all_model_collapsed = nan(p.NUMBER_OF_CONDITIONS, p.NUMBER_OF_CONDITIONS);
    ...
    for p1 = 1:p.NUMBER_OF_CONDITIONS
        ...
        for p2 = (p1):p.NUMBER_OF_CONDITIONS
            ...
            values = colour_average_all_model_nodiag(ind_p1, ind_p2);
            value = nanmean(values(:));
            ...
            colour_average_all_model_collapsed(p1, p2) = value;
            colour_average_all_model_collapsed(p2, p1) = value;
        end
    end
    
%% Collapsed Model Figure
    fig = figure('Position', [1 1 1200 1000]);
    fprintf('\nCreating collapsed colour average (all) model figure...\n')
        clf
        imagesc(colour_average_all_model_collapsed)
        axis image
        colormap jet
        colorbar
        caxis([0 1])
...
%% Save Collapsed Model
    fprintf('\nSaving collapsed colour average (all) model...\n')
    fp = [DIR_SAVE FILENAME_SAVE '_collapsed' '.mat'];
    fprintf('Filepath: %s\n', fp);
    save(fp, 'colour_average_all_model_collapsed');
